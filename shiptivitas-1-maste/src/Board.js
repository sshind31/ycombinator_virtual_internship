...
export default class Board extends React.Component {
  ...
  async getClients() {
    let res = await axios.get('http://localhost:3001/api/v1/clients',);
    return res.data;
  }
  // axios
  ...
  componentDidMount() {
    axios.get('http://localhost:3001/api/v1/clients',).then(res => {
      this.setState({
        clients:
        {
          backlog: res.data.filter(client => !client.status || client.status === 'backlog'),
          inProgress: res.data.filter(client => client.status && client.status === 'in-progress'),
          complete: res.data.filter(client => client.status && client.status === 'complete'),
        }
      })
    });
    // this.setState({ clients: reqData.data })
  ...
  updateClient(el, target, _, sibling) {
    ...
    console.log(clientThatMovedClone.status + " $$ " + clientThatMovedClone.name + " $$ " + clientThatMovedClone);
